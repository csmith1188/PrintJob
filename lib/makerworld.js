...
function instancePlateCount(instance) {
  const ext = instance?.extention || instance?.extension || {};
  const plates = ext.modelInfo?.plates;
  if (Array.isArray(plates) && plates.length > 0) return plates.length;
  const n = Number(instance?.plateCnt ?? instance?.plateCount);
